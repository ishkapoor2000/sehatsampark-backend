...
def create_camps():
    if request.method == "POST":
        ...
        print("camp_pincode", camp_pincode)
...
def send_sms(pincode, distance=5):
    ...
    nearby_areas.append(str(pincode))
    ...
    # camp = camps_collection.find({"venue.pin_code": str(pincode)})

    # camp_name = camp.get('camp_name')
    # camp_complete_address = camp.get('venue', {}).get('complete_address')
    # camp_start_time = camp.get('time_period', {}).get('start_time')
    # camp_end_time = camp.get('time_period', {}).get('end_time')

    # print(camp_name, camp_complete_address, camp_start_time, camp_end_time)

    for user in m_users:
        print(user['gender'], user['phone_number'], user['full_name'])
        # send_greeting_sms(user['phone_number'], user['full_name'], camp_name, camp_complete_address, camp_start_time, camp_end_time)
        send_greeting_sms("+919289708244", user["full_name"], camp_name="ABC", camp_complete_address="BPIT College", camp_start_time="10:00 AM", camp_end_time="10:00 PM")
        print(f">>> SMS sent to {user['full_name']}.")
